      ...
      <div className="flex justify-between items-start w-full">
        ...
        <div className="flex gap-2">
          {orders.map((order) => (
            <Card
              key={order.id}
              className="w-24 h-28 bg-white/90 border-2 border-slate-300 shadow-xl overflow-hidden animate-in slide-in-from-right fade-in duration-300"
            >
              <CardContent className="p-2 flex flex-col items-center justify-between h-full">
                ...
                <Progress
                  value={(order.timeRemaining / order.totalTime) * 100}
        ...
        <div className="absolute inset-0 bg-black/70 flex items-center justify-center pointer-events-auto animate-in fade-in duration-500">
          <Card className="w-[500px] border-4 border-yellow-500 bg-slate-900 text-white shadow-2xl">
            <CardContent className="flex flex-col items-center gap-6 p-10">
              <h2 className="text-4xl font-extrabold text-yellow-400 drop-shadow-lg tracking-wider">
                LEVEL COMPLETE!
              </h2>
